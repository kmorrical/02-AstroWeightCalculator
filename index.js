...
/* This code will take away Pluto when the box is checked */
   function nPluto() {
        if (document.getElementById('checkbox').checked) {
            var list = document.getElementById("selectPlanet");   // Get the <ul> element with id="myList"
            list.removeChild(list.childNodes[10]);           // Remove <ul>'s first child node (index 0)
        } 
       else /*This code will bring Pluto back when box is unchecked*/{ 
            var selectElement = document.getElementById('selectPlanet'); //adds planets list elements to list
            selectElement.appendChild(optionElement);
